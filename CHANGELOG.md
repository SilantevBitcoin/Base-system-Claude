...
### Добавлено
- **Move 8 «Соседи перед правкой» в дисциплину кода** (`rules/coding-discipline.md`): перед правкой
  элемента, делящего с другими общий ограниченный ресурс (место в layout · общий родитель · общее
  значение/состояние · автоматика на связанном — FK-каскад/триггер/вычисляемое поле), — секундный
  взгляд «кто рядом и что с ним станет», с гашением нежелательного эффекта сразу выбором пути правки.
  Защита от ripple первого порядка; действует на всех ставках. Лёгкий младший брат blast-radius
  (Move 7): дешёвый «глаз» для частых мелочей там, где тяжёлый LSP-анализ избыточен.
