...
# Production: multiple workers with concurrency and priority queues
...
### Retryable Task (Exponential Backoff + Jitter)
...
### Idempotent Task (Status Guard)

Design tasks so they can safely run multiple times with the same inputs — delivery is at-least-once.
...
def generate_pdf_report(self, report_id: int) -> str:
    ...
    try:
        ...
        PDFGenerator.cleanup(report_id)   # Clean up partial files before hard kill
        raise
```
...
parallel = group(send_welcome_email.s(uid) for uid in new_user_ids)
parallel.delay()
...
```python
# apps/core/tasks.py — central failure capture
...
def on_task_failure(sender, task_id, exception, args, kwargs, traceback, einfo, **kw):
    ...
    with sentry_sdk.new_scope() as scope:
        scope.set_context('celery', {
            'task': sender.name, 'task_id': task_id, 'args': args, 'kwargs': kwargs,
        })
        sentry_sdk.capture_exception(exception)
```

```python
# Route failed tasks to a dead-letter table after max retries
...
def charge_card(self, order_id: int) -> None:
    ...
    try:
        ...
        if self.request.retries >= self.max_retries:
            ...
            FailedCharge.objects.create(order_id=order_id, error=str(exc), task_id=self.request.id)
...
class TestSendWelcomeEmail:
    ...
    def test_skips_missing_user_gracefully(self):
        """Must not raise when the entity is deleted between enqueue and execute."""
        send_welcome_email(99999)
```

### Integration Testing (run tasks eagerly)
...
send_welcome_email.delay(user.pk)     # Always pass PKs/IDs

# BAD: Calling tasks synchronously in production request handlers
...
# GOOD: Idempotent with status guard + row lock
@shared_task
def charge_and_fulfill(order_id):
...
| Failure capture (Sentry/alerting) | `task_failure` signal handler |
| Dead-letter path for permanent failures | Persist + alert, do not silently drop |
| Monitor (Flower or equivalent) | Visibility into queue depths |
