...
Works well with: `secrets-management` (inject secrets into containers without baking them into layers), `grpc-golang` / application skills (containerize the service), `prometheus-configuration` (expose `/metrics` + container resource metrics). Kubernetes orchestration and cloud deployment are devops-column scope.
