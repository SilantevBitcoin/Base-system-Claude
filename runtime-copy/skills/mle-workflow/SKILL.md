...
## MLE Is Software Engineering With Stricter Failure Modes

Do not treat MLE as separate from software engineering. Most SWE discipline applies directly to ML systems, often with stricter failure modes:

- Scan existing training, feature, serving, eval, and monitoring paths before introducing a parallel ML stack.
- Scope model changes as product capabilities with data, eval, serving, and rollback phases — not as "improve the model".
- Test feature transforms, split logic, metric calculations, artifact loading, and inference schemas before implementation.
- Review for ML-specific risks on top of normal code review: leakage, reproducibility, promotion-gate correctness, and monitoring coverage.
- Require automated evidence for transforms, metrics, inference contracts, promotion gates, and rollback behavior.
- Exclude secrets and PII from model artifacts, notebooks, prompts, datasets, and logs.
